// Copyright 2026 The MuTate Contributors
// SPDX-License-Identifier: MIT OR Apache-2.0

//! # Bank
//!
//! > Get to the chopper!
//! >
//! > Jean Claude Van Damme
//!
//! Chop up spectrum into bins.  Tell us important facts about each bin.
//!
//! When designing a filter bank, the geometry of the bins is usually selected first and then
//! filters are chosen to fit the bank.  This module is focused on the geometry part, deciding where
//! bin boundaries will be.

use super::iso226;

#[derive(Debug)]
pub struct Bin {
    /// Minimum frequency
    pub min: f64,
    /// Maximum frequency
    pub max: f64,
    /// Center frequency
    pub center: f64,
    /// Linear amplitude factor (>= 1.0) to pre-multiply into this bin's window weights so the
    /// measured magnitude comes out perceptually flat on the 70 phon curve.
    pub iso226_gain: f64,
}

impl Bin {
    /// Difference between minimum and maximum frequency.
    pub fn bandwidth(&self) -> f64 {
        self.max - self.min
    }

    /// Quality factor for this bin presuming we have a perfect filter.
    pub fn q(&self) -> f64 {
        self.center / self.bandwidth()
    }
}

/// Return a list of bin spacings.  We use logarithmic spacing because it matches music and pretty
/// closely matches human senses of tones.  `min` and `max` are frequencies in Hz.  Count is the
/// number of bins.
pub fn bins(min: f64, max: f64, count: usize) -> Vec<Bin> {
    assert!(max > min);
    assert!(count > 1);

    // Once again, I shall remind myself how to walk the log.  Just a terrible morning and I haven't
    // yet digested the coffee. ☕
    //
    // The sum of logs is the log of a product.  The product of all of the factors for all steps is
    // equal to `max / min`.  We are chopping up a ratio of `(max / min)` into equal ratios rather
    // than chopping up a sum into equal summands.
    //
    // `min` and `max` are centers, so the walk spans count - 1 whole steps.  Bins must know their
    // width, so we walk in half steps and let the outer half steps hang outside the range.
    let ratio = max / min;
    let steps = 2 * (count - 1);

    // method 1
    // max_ratio: 1.0000000000000004441
    let log_step = ratio.log2() / steps as f64;
    let freq = |i: i64| min * (log_step * i as f64).exp2();

    // Method 2
    // max_ratio: 0.9999999999997188915
    // sum of logs is log of product, and the product of steps is equal to the ratio.
    // let step = (ratio.log2() / (steps as f64)).exp2();
    // let freq = |i: i64| min * step.powi(i as i32);

    // Method 3
    // max_ratio: 0.9999999999994294564
    // The ith root to the ith power is unity
    // let step = ratio.powf(1.0 / steps as f64); //  * 0.99999989418598;
    // let freq = |i: i64| min * step.powf(i as f64);

    // Method 2
    (0..count)
        .map(|i| {
            let i0 = 2 * i as i64;
            let center = freq(i0);
            Bin {
                min: freq(i0 - 1),
                center,
                max: freq(i0 + 1),
                iso226_gain: iso226::iso226_window_gain(center),
            }
        })
        .collect()
}

pub fn bin_lookup(min: f64, max: f64, count: usize, center: f64) -> Bin {
    let mut bins = bins(min, max, count);
    let mut closest = 0usize;
    let mut min_dist = f64::MAX;
    for (i, b) in bins.iter().enumerate() {
        let ratio = center / b.center;
        let dist = (1.0 - ratio).abs();
        if dist < min_dist {
            closest = i;
            min_dist = dist;
        }
    }
    bins.remove(closest)
}

#[cfg(test)]
mod test {
    use crate::dsp;

    use super::*;

    #[test]
    fn bins_are_sane() {
        let count = dsp::spectrogram::RESOLUTION_4K_WIDTH;
        let freq_min = dsp::MIN_FREQ_CHEAP_DRIVERS;
        let freq_max = dsp::MAX_FREQ_OLD_PEOPLE;
        let bins = bins(freq_min, freq_max, count);

        assert_eq!(bins.len(), count);

        let min = bins[0].center;
        let max = bins.last().unwrap().center;

        // Check that we covered the range by verifying that our interpolation hits the same
        // beginning and end points.
        let min_ratio = min / dsp::MIN_FREQ_CHEAP_DRIVERS;
        let max_ratio = max / dsp::MAX_FREQ_OLD_PEOPLE;

        // println!("min ratio: {:10.19}", min_ratio);
        // println!("max ratio: {:10.19}", max_ratio);

        // So basically we are testing for more accuracy than what f32 can reliably represent.  Nice
        // bins for hardcoding ;-)
        assert!((min_ratio - 1.0).abs() < 0.000000000000001);
        assert!((max_ratio - 1.0).abs() < 0.000000000000001);
        assert_eq!(bins.len(), count);

        // Well ordered
        for b in bins.iter() {
            assert!(b.min < b.center);
            assert!(b.center < b.max);
        }

        // Bins tile their covered span
        let mut sum = 0.0;
        for b in bins.iter() {
            sum += b.bandwidth();
        }
        let span = bins.last().unwrap().max - bins[0].min;
        assert!(((sum / span) - 1.0).abs() < 0.000000000000001);
    }

    #[test]
    fn bin_lookup_is_sane() {
        let target = 4000.0;
        let closest = bin_lookup(
            dsp::MIN_FREQ_CHEAP_DRIVERS,
            dsp::MAX_FREQ_OLD_PEOPLE,
            dsp::spectrogram::RESOLUTION_4K_WIDTH,
            target,
        );

        let ratio = closest.center / target;
        assert!((ratio - 1.0).abs() < 0.01);

        // First bin is centered on the minimum frequency
        let closest = bin_lookup(
            dsp::MIN_FREQ_CHEAP_DRIVERS,
            dsp::MAX_FREQ_OLD_PEOPLE,
            dsp::spectrogram::RESOLUTION_4K_WIDTH,
            0.0,
        );
        let ratio = closest.center / dsp::MIN_FREQ_CHEAP_DRIVERS;
        assert!((ratio - 1.0).abs() < 0.0000001);

        // Last bin is centered on the maximum frequency
        let closest = bin_lookup(
            dsp::MIN_FREQ_CHEAP_DRIVERS,
            dsp::MAX_FREQ_OLD_PEOPLE,
            dsp::spectrogram::RESOLUTION_4K_WIDTH,
            100_000.0,
        );
        let ratio = closest.center / dsp::MAX_FREQ_OLD_PEOPLE;
        assert!((ratio - 1.0).abs() < 0.0000001);
    }
}
