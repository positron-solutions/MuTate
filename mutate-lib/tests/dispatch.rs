...
fn dispatch_increment_read_back() {
    ...
    with_context!(|device, _instance| {
        ...
        let pipeline = ComputePipeline::<IncrementPipeline>::new(&device).unwrap();
        pipeline.push(
            &device,
        ...
        pipeline.dispatch(&device, *cb, 1, 1, 1);
        ...
        wait_value.wait(&device, 100_000_000).unwrap();
        output_buffer.invalidate(&device);
